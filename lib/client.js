...
Client.prototype.addSoapHeadersInside = function(name, value) {
  if (!this.soapHeadersInside) {
    this.soapHeadersInside = [];
  }
  this.soapHeadersInside.push(name+'="'+value+'"');
};
Client.prototype.getSoapHeadersInside = function() {
  if (!this.soapHeadersInside) {
    this.soapHeadersInside = [];
  }
  return this.soapHeadersInside;
};
Client.prototype.clearSoapHeadersInside = function() {
  this.soapHeadersInside = [];
};
...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  xml = "<?xml version=\"1.0\" encoding=\"utf-8\"?>" +
    ...
    encoding +
    this.wsdl.xmlnsInEnvelope + (self.additionalEnvelope ? self.additionalEnvelope : "") +  '>' +
    ((self.soapHeaders || self.security) ?
      (
        "<" + envelopeKey + ":Header"+(self.getSoapHeadersInside() ? " "+self.getSoapHeadersInside().join(" ") : "")+">" +
